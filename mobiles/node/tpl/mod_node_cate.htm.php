...
<script>
gData.referURI='/';
function showNodeCateList(obj) {
	if (typeof (showNodeCateList.target) == 'undefined') {
		showNodeCateList.target = $('#nodeCateList');
	}

	$('#opSortList').attr('data-switch','close');
	$('#nodeSortList').hide();
	
	if ($(obj).attr('data-switch')=='close') {
		$(obj).attr('data-switch','open');
		showNodeCateList.target.slideDown();
	}else{
		$(obj).attr('data-switch','close');
		showNodeCateList.target.slideUp();
	}
	return false;
}
function showNodeSortList(obj) {
	if (typeof (showNodeSortList.target) == 'undefined') {
		showNodeSortList.target = $('#nodeSortList');
	}

	$('#opCateList').attr('data-switch','close');
	$('#nodeCateList').hide();
	
	if ($(obj).attr('data-switch')=='close') {
		$(obj).attr('data-switch','open');
		showNodeSortList.target.slideDown();
	}else{
		$(obj).attr('data-switch','close');
		showNodeSortList.target.slideUp();
	}
	return false;
}
</script>

<div class="typeli clearfix">
  <a href="javascript:void(0);" class="op" id="opCateList" onclick="return showNodeCateList(this)" data-switch="close"><?=$the_cate_name?><b></b></a>
  <a href="javascript:void(0);" class="op" id="opSortList" onclick="return showNodeSortList(this)" data-switch="close"><?=$orderby_name?><b></b></a>
  <ul class="bbsizing nodeCateList" id="nodeCateList">
    <li><a href="#/cate/<?=$the_cate_id?>/0/,t=<?=$the_type_id?>" <?php if(!$the_tag_id):?>class="checked"<?php endif;?>>全部</a></li>
    <?php foreach ($tag_list AS $it):?>
    <li><a href="#/cate/<?=$the_cate_id?>/<?=$it['tag_id']?>,t=<?=$the_type_id?>" <?php if($the_tag_id==$it['tag_id']):?>class="checked"<?php endif;?>><?=$it['tag_name']?></a></li>
    <?php endforeach;?>
  </ul>
  <ul class="bbsizing nodeSortList" id="nodeSortList">
    <li><a href="#/cate/<?=$the_cate_id?>,orderby=created" <?php if($orderby=='created'):?>class="checked"<?php endif;?>>最新发布</a></li>
    <li><a href="#/cate/<?=$the_cate_id?>,orderby=votecnt" <?php if($orderby=='votecnt'):?>class="checked"<?php endif;?>>点赞最多</a></li>
    <li><a href="#/cate/<?=$the_cate_id?>,orderby=collectcnt" <?php if($orderby=='collectcnt'):?>class="checked"<?php endif;?>>收藏最多</a></li>
  </ul>
</div>
<?php endif;?>

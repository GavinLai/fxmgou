...
<?php if(in_array('blk_1', $show_blk)):?>
<div class="typeinfo w90">
<form id="search" action="#/node/search" method="get" onsubmit="return false;">
  <span >
     <input type="text" value="<?=$keywords?>"  id="keywords" name="keywords" class="bbsizing" placeholder="输入关键字" />
  </span>
  <span>
    <input type="submit" value="查询" id="find_btn" class="bbsizing"/>
  </span>
</form>
...
<?php if ($it['type_id']=='word'):?>
      <div class="bbsizing typelist2" data-type="<?=$it['type_id']?>" data-nid="<?=$it['nid']?>" data-content="<?=$it['content']?>" data-img="">
...
<?php elseif ($it['type_id']=='card'):?>
  <div class="bbsizing typelist2" data-type="<?=$it['type_id']?>" data-nid="<?=$it['nid']?>" data-content="<?=$it['content']?>" data-img="<?=$it['cover_url']?>">
...
<?php elseif ($it['type_id']=='music'):?>
      <div class="bbsizing typelist2" data-type="<?=$it['type_id']?>" data-nid="<?=$it['nid']?>" data-content="<?=$it['title']?>-<?=$it['singer_name']?>" data-img="<?=$it['icon_url']?>">
...
<?php elseif ($it['type_id']=='gift'):?>
    <div class="bbsizing typelist2" data-type="<?=$it['type_id']?>" data-nid="<?=$it['nid']?>" data-content="<?=$it['title']?> - <?php echo truncate((strip_tags($it['desc'])),30)?>" data-img="<?=$it['goods_url']?>">
...
$(document).ready(function(){
  ...
  $('#search').bind('submit',function (){
    ...
    var hash = '#/node/search,keywords='+keywords;
    go_hashreq(hash);
    return false;
  });
});
